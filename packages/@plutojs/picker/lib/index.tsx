...
class Group extends Component<GroupProps, GroupState> {
  ...
  private setTranslateY = () => {
    ...
    const itemHeight = (this.item as HTMLElement).getBoundingClientRect()
      .height;
    const translateY =
      (containerHeight - itemHeight) / 2 - itemHeight * selectedIndex;
  ...
  private computeTranslateY = (e: React.TouchEvent) => {
    ...
    if (distanceY < 0 && Math.abs(distanceY) > maxUpDistanceY)
      distanceY = -maxUpDistanceY;

    // 计算可下移最大距离
    const maxDownDistanceY = selected * this.itemHeight;
    if (distanceY > 0 && distanceY > maxDownDistanceY)
      distanceY = maxDownDistanceY;

    // 可移动规围内取整
    distanceY =
      parseInt((distanceY / this.itemHeight).toFixed(0)) * this.itemHeight;
  ...
  private computeSelected = (translateY: number) => {
    ...
    const selected = parseInt(
      (Math.abs(translateY - baseTranslateY) / this.itemHeight).toFixed(0)
    );
    return selected;
  };
    ...
    return (
      ...
      >
        <div
          ...
          ref={(item) => {
            this.scrollContainer = item;
          }}
        >
          {data.map((item, index) => (
            <div
              ref={(item) => {
                if (index === 0) this.item = item;
              }}
              key={`item-${index}`}
...
class Picker extends Component<PickerProps, PickerState> {
  ...
  private initGroupItems = (
    ...
    if (index < group) {
      const children = selected[index]
        ? items.find((item) => item.value === selected[index].value).children
        : items[0].children;
      groupItems = groupItems.concat(
        this.initGroupItems(group, children, selected, index + 1)
      );
      return groupItems;
    }
    return groupItems;
  ...
  private onChange = (item: PickerItemType, index: number) => {
    if (index < this.selected.length - 1) {
      ...
      const newGroupItems = groupItems
        .slice(0, index + 1)
        .concat(this.initGroupItems(group - index - 1, item.children || []));
      this.setState({
          ...
          >
            ...
            {showGroup && groupItems.length > 0 && (
              <div className={`${style.scrollInner}`}>
                {groupItems.map(
                  (items: Array<PickerItemType>, index: number) => (
                    <Group
                      key={`item-${index}`}
